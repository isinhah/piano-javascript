...
//Função pra adicionar addEventListener de click em cada tecla
keys.forEach((key) => {
  key.addEventListener("mousedown", () => handleMouseDown(key));
  key.addEventListener("mouseup", () => handleMouseUp(key));
});

//Muda a cor do botão de Show keys, quando ativado ou desativado
...
const keyDownMapper = {
  ...
  z: () => handleMouseDown(keys[22]),
  x: () => handleMouseDown(keys[23]),
};

const keyUpperMapper = {
